  ...
  if (isLoading) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">Fetching order details for checkout...</p>
      </div>
    );
  }
  ...
  if (!order) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-20 text-center space-y-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">The requested rental order checkout session is invalid.</p>
        <Link href="/gear" className="inline-flex items-center space-x-2 px-4 py-2 rounded-xl text-xs font-bold text-white bg-slate-900 dark:bg-emerald-600 hover:bg-slate-800 dark:hover:bg-emerald-500 transition-all">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-6">
        <div>
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
            Rental <span className="text-emerald-600 dark:text-emerald-400">Checkout</span>
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Review your order breakdown and complete secure Stripe payment in BDT (৳).</p>
        </div>
        <div className="flex items-center space-x-2 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 px-3.5 py-1.5 rounded-full text-xs font-semibold text-emerald-700 dark:text-emerald-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-5">
            <h3 className="text-base font-bold text-slate-900 dark:text-white border-b border-slate-100 dark:border-slate-800 pb-3">Rental Item Summary</h3>
            <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
              {order.gear?.imageUrl && (
                <div className="relative w-full sm:w-28 h-28 rounded-xl overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shrink-0">
                  <Image src={order.gear.imageUrl} alt={order.gear.title || 'Gear'} fill className="object-cover" />
                </div>
              )}
              <div className="space-y-1.5 flex-1">
                <span className="text-[11px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 px-2.5 py-0.5 rounded-full">
                  Order ID: {order.id.slice(0, 12)}...
                </span>
                <h4 className="text-lg font-bold text-slate-900 dark:text-white">{order.gear?.title}</h4>
                <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-2">{order.gear?.description}</p>
                <div className="flex items-center space-x-2 text-xs text-slate-500 dark:text-slate-400 pt-1">
                  <MapPin className="w-3.5 h-3.5" />
                  <span>{order.gear?.location || 'Dhaka, Bangladesh'}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1">
                ...
                <p className="text-sm font-bold text-slate-900 dark:text-white">{order.startDate}</p>
              </div>
              <div className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl border border-slate-200 dark:border-slate-700 space-y-1">
                <p className="text-xs text-slate-500 dark:text-slate-400">End Date</p>
                <p className="text-sm font-bold text-slate-900 dark:text-white">{order.endDate}</p>
              </div>
            </div>
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 text-xs">
              ...
              <span className="font-bold text-emerald-600 dark:text-emerald-400 text-sm">{order.totalDays} Days</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-1">
          <div className="bg-slate-900 dark:bg-slate-900/90 p-6 rounded-2xl border border-slate-800 shadow-xl space-y-6 sticky top-24">
            ...
            <div className="flex items-center space-x-2 text-[11px] text-slate-400 bg-slate-800/80 p-3 rounded-xl border border-slate-700">
